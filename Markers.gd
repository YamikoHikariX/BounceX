...
var gap_label_scene: PackedScene = preload('res://gap_label.tscn')
		

func _input(event):
	if event.is_action_pressed('center_marker'):
		center_selected_marker()
	if event.is_action_pressed('show_gaps'):
		are_gaps_shown = true
	elif event.is_action_released('show_gaps'):
		are_gaps_shown = false
...
func center_selected_marker():
	if not selected_marker:
		return
	var next_marker = get_next_marker(selected_marker.frame)
	var previous_marker = get_previous_marker(selected_marker.frame)
	var distance = get_distance(previous_marker, next_marker)
	var middle: int = previous_marker.frame + distance / 2
...
func get_visible_markers() -> Array[Marker]:
	...
	for marker in marker_list.values():
		if marker.global_position.x > 0 and marker.global_position.x < visible_rect.size.x:
			markers.append(marker)
	return markers
		

func hide_gaps():
	for node in %GapLabels.get_children():
		node.queue_free()

func get_distance(marker_a, marker_b):
	return abs(marker_a.frame - marker_b.frame)
...
func marker_toggled(button_pressed:bool, marker:Node):
	...
	if button_pressed:
		if owner.shift_pressed and selected_marker or selecting_to_edge:
			...
			var index_a = get_marker_index(selected_marker.frame)
			var index_b = get_marker_index(marker.frame)
			var selected_range
			if index_a < index_b:
		...
		depth_input.value = marker.depth
		depth_input.value_changed.connect(_on_depth_value_changed)
		%MarkersMenu/HBox/Trans.select(marker.trans)
		%MarkersMenu/HBox/Ease.select(marker.ease)
	else:
		frame_input.value_changed.disconnect(_on_frame_value_changed)
		depth_input.value_changed.disconnect(_on_depth_value_changed)
	...
	for set in sequences:
		...
		if previous_frame == set.front():
			...
			if next_frame != set.back():
				max_frame = get_next_marker_frame(set.back()) - SEPARATION_MIN
			else:
...
func connect_marker(frame:int, connect_next:=true) -> void:
	...
	var marker: Marker = marker_list[frame]
	var previous: Marker = marker_list[previous_frame]
	var starting_position = previous.position
	if connect_next and next_frame != frame:
		connect_marker(next_frame, false)
	if marker.line:
		remove_child(marker.line)
		marker.line.queue_free()
	var line = $Line.duplicate()
	add_child(line)
	line.add_to_group('lines')
	marker.line = line
	var tween = get_tree().create_tween()
	tween.set_trans(marker.trans)
	tween.set_ease(marker.ease)
	var steps = marker.frame - previous.frame
	tween.tween_property(previous, 'position:y', marker.position.y, steps)
	tween.pause()
	line.clear_points()
	var line_frame = previous.frame
	for i in steps + 1:
...
func _on_depth_value_changed(value):
	...
	for marker in markers:
		if not owner.marker_data.has(marker.frame):
			return
		var new_pos = marker.depth + confined_movement
		marker.position.y = owner.BOTTOM + new_pos * (owner.TOP - owner.BOTTOM)
		marker.depth = new_pos
		owner.marker_data[marker.frame][0] = new_pos
		if marker.frame == 0:
			connect_marker(get_next_marker_frame(0))
		connect_marker(marker.frame)
		place_ball_on_path()
		Data.save_path()
...
func _on_copy_pressed():
	...
	if not selected_multi_markers.is_empty():
		for marker in selected_multi_markers:
			...
			frame_list.append(marker.frame)
	
	frame_list.sort()
	var starting_frame = frame_list.front()
	
	for marker in selection:
		clipboard[marker.frame - starting_frame] = [
			marker.depth,
			marker.trans,
			marker.ease,
			marker.auxiliary]
